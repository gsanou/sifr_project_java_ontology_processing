...
public class SQLUMLSDelegate implements UMLSDelegate {
    ...
    private static final String CONCEPT_STR_PREFIX = "cuistr_";
    ...
    public SQLUMLSDelegate(final String jdbcURI, final String sqlUser, final String sqlPass, final String sqlDB, final JedisCommands jedis) {
                ...
                synchronized (dataSource) {
                    try (final Connection connection = dataSource.getConnection()) {
                        try (final PreparedStatement statement = connection.prepareStatement("SELECT DISTINCT TUI FROM MRSTY WHERE CUI=?")) {
                            ...
                            localTuis = fetchPushListPreparedStatement(statement, key);
    ...
    private Collection<String> fetchPushListPreparedStatement(final PreparedStatement preparedStatement, final String cacheKey) throws SQLException {
    ...
    private String buildCUIDescriptionQuery(final int numberOfCUIs) {
        String query = "SELECT DISTINCT CUI,STR FROM MRCONSO WHERE LAT=?";
        query += " AND (";
        boolean first = true;
    ...
    @Override
    public Collection<String> getUMLSCUIs(final UMLSLanguageCode languageCode, final String cui, final String strValue) {
        final String code = languageCode.getLanguageCode();
        final String key = CONCEPT_STR_PREFIX + languageCode.getLanguageCode() + strValue;
        Collection<String> cuis = jedis.lrange(key, 0, -1);
        if (cuis.isEmpty() && !EmptyResultsCache.isEmpty(key, jedis)) {
            //noinspection SynchronizeOnNonFinalField
            synchronized (dataSource) {
                try (final Connection connection = dataSource.getConnection()) {
                    try (final PreparedStatement statement = connection.prepareStatement("SELECT DISTINCT CUI FROM MRCONSO WHERE CODE IN (SELECT CODE FROM MRCONSO WHERE LAT=? AND STR=? AND CUI=?) AND LAT=?")) {
                        statement.setString(1, code);
                        statement.setString(2, strValue);
                        statement.setString(3, cui);
                        statement.setString(4, code);
                        cuis = fetchPushListPreparedStatement(statement, key);
                    } catch (final SQLException e) {
                        logger.error(ERROR_MESSAGE_CANNOT_RUN_SQL_QUERY, e.getLocalizedMessage());
                    }
                } catch (final SQLException e) {
